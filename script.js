...
function init() {
  ...
  */
  ...
  scene.fog = new THREE.Fog(0x0f0f1e, 50, 100);
...
function startGame() {
  ...
  // Hide modals
  if (instructionsElement) instructionsElement.classList.remove("show");
...
function splitBlockAndAddNextOneIfOverlaps() {
  ...
  if (overlap > 0) {
    ...
    if (levelElement) levelElement.innerText = `Level: ${stack.length}`;
  } else {
    missedTheSpot();
  }
}
...
function missedTheSpot() {
  ...
  if (resultsElement && !autopilot) {
    setTimeout(() => {
      if (finalScoreElement) finalScoreElement.innerText = stack.length - 1;
